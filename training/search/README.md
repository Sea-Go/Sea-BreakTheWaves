...
**固定 BGE-M3 三路基线（可选同次验收）。** 在上面的父入口增加 `--three-lane-model-directory <已按model.lock锁定的本地BAAI/bge-m3目录> --bge-python training/serving/bge_m3/.venv/bin/python`，父进程保持 v2 SeaweedFS 在线，调用独立 qrel reader 与官方固定权重生成 Dense、Sparse、Token Matrix 的原值/内容寻址清单，对 train/validation/test 分别按时间可用的全量冻结chunk计算三路候选排序，再让现有 Go exact 检索器用同一原值 Build→Search 做 test 分片数值/TopK 对账。稀疏倒排正式语义只返回正分命中，Python原始全候选中的零分须单列：逐项原分数仍对账，TopK只在同一正分候选口径比较，不能宣称全候选TopK无差别。报告留在父证据目录的 `three-lane/`。所有 qrel 为合成判定且没有全候选已判覆盖收据，正式 Recall/MRR/nDCG 固定 `not_evaluable`；该基线不训练三路权重，也不激活线上模型或索引。
