...
func validate(deps Dependencies) error {
	if deps.Model == nil {
		return fmt.Errorf("search agent model is required")
	}
	if deps.Instruction == "" {
		return fmt.Errorf("search agent instruction is required")
	}
	return nil
}

func NewSearchAgent(deps Dependencies) (trpcagent.Agent, error) {
	...
	return llmagent.New("sea_search", opts...), nil
}
