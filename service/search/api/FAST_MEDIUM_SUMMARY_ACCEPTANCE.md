...
审查另记P3：原通用JSON Decoder允许新增`fast_medium`键双写或`\u0066ast_medium`同义拼写；在有效中档对象后附`null`会把解析值覆成nil，使正式进程静默只宣告低档。现只在策略文件上用逐Token扫描和原始键字节比对固定新增对象的唯一字面量键及七个内部键，显式`null`拒启动；旧通用索引`readJSON`以及没有`fast_medium`的旧`fast_low`政策解析路径不变。配置反例包括有效对象后`null`、Unicode转义根别名、内部版本重复/别名与普通旧低档，均由同一有效/无效政策门禁签收。后续合前源复核又找到大小写同义根键：Go `encoding/json`会把`"FAST_MEDIUM":null`和`"\u0046ast_medium":null`匹配到同一结构字段，原Token层仅精确判断`key=="fast_medium"`会漏检并把合法中档覆成仅低档。补修后Token层先以`strings.EqualFold`识别所有可能绑定新增根字段的拼写，再只准唯一字面量小写`"fast_medium"`；内部七个字段本就只准唯一小写字面量，也用`VERSION`及转义大写版本反例核其大小写别名不得覆值。旧低档/索引解析保持原路径。
...
## 合前再审：大小写同义配置与未公开有效等级

策略专用Token层曾仅识别解码后精确小写`fast_medium`，但Go `encoding/json`结构解码会大小写折叠地接受`"FAST_MEDIUM"`或`"\u0046ast_medium"`。有效中档对象后附这些别名的`null`会绕过Token层，把配置再次静默覆为仅低档。现Token先以`strings.EqualFold`识别所有可绑定新增根字段的拼写，再按原字节只接受唯一字面量小写`"fast_medium"`；合法对象及后附大小写/转义大小写别名、内部`VERSION`/转义大写版本反例均在同一策略专项通过。旧没有中档的低档JSON与索引`readJSON`路径不改变。

正式 Summary 的外层HTTP/RTW转发当前没有 `effective_intelligence`、`policy_version`、`change_reason`。旧`Search.Service.effective`按RTW签发的`AllowLowerIntelligence=true`允许缺Medium时改用Low，或High改用Medium/Low；若仍返回HTTP200，用户看到“中/高”而实际只跑低/中，违反D12的明示降级合同。此候选**在Runner前对实际Service有效Profile要求 `EffectiveIntelligence == RequestedIntelligence`**，即使allow-lower由RTW签发也拒隐式降级→503 `SEARCH_PROFILE_UNAVAILABLE`；原Low请求依旧走单查询/200。其他SearchExecutor若无法做策略前置检查，根Summary在框架结果出边界前再拒下移结果，避免公开假等级；其检索/模型/引用可能已经发生，因此正式装配仍须使用有前置Profile能力的`Search.Service`才能签零副作用。旧已接纳历史的读取校验不重写。真正允许降级需RTW/Web发布并实际展示有效等级、策略版本和原因的另版外层合同，本切片不先开启。受控HTTP夹具固定`AllowLowerIntelligence=true`分别验Medium→Low与High→Low的503、模型/三路/原文/引用/历史零新增；原生根Graph在Low+Medium政策下验证High→Medium也在模型/三路前拒；另用无前置能力的受控ExecuteFunc降级结果证明不会公开答案或进入接纳历史。

合前再审后的**最终本分支源码**执行三包`-race`退出0，日志SHA-256=`a5e58e372568caef86e9c55744c1cf69bbc22d7eadf775d56b2f69107e1fa0c6`；`go test -p 1 -mod=readonly -count=1 ./...`43包退出0，SHA-256=`499026508115e9205110601879f4459facd20093bfdb7ab735299cf46ff8933f`。政策正/大小写转义别名反例专项退出0，SHA-256=`efb223f7bd1c1fb7e84e7dd88e2ab654e1ffabf492879432bd4c39604256b8b8`；根Graph等级正/负及无Profile执行器出边界专项退出0，SHA-256=`7fda7edf9cb426c8d3bf807306a2f3587436696c2cfc3cebf99e75903c4ce3c3`。全仓`go vet -p 1 ./...`、`go mod verify`、`git diff --check`退出0。以上证明的是受控Scope、固定模型/三路/历史夹具上的**策略不偷降及公开结果门禁L1**，不证明RTW真HMAC签发、真实规划模型/索引、Web实际等级展示或生产默认可用。
