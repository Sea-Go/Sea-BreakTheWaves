...
const (
	...
	ProductAction = contract.ProductAction
	Reading       = contract.Reading
	Impression    = contract.Impression
	LocalSemantic = contract.LocalSemantic
	SelfReport    = contract.SelfReport
)
...
func (s *Store) Append(ctx context.Context, input Event) (receipt Receipt, err error) {
	...
	var v2 SubjectRefV2
	if s.v2Candidate {
		v2, err = subjectV2Projection(input.Subject)
		if err != nil {
			return Receipt{}, err
		}
	}
