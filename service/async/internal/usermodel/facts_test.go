...
func fixture(id string, sequence int64) Event {
	...
	return Event{Subject: SubjectRef{AuthorityID: "rtw.identity", TenantID: "tenant-a", SubjectID: "user-1"},
		EventKey: EventKey{Producer: "rtw.product", EventID: id}, Action: Assert, Kind: Reading,
...
func TestPostgresPendingCycleRejectedWithoutFalseAcceptance(t *testing.T) {
	s := testStore(t, nil)
	a := fixture("cycle-a", 1)
	a.Action, a.Supersedes = Correct, &EventKey{Producer: "rtw.product", EventID: "cycle-b"}
...
func TestPackageObservabilityWithInjectedProcessBundle(t *testing.T) {
	...
	pending.Action, pending.Supersedes = Correct, &EventKey{Producer: "rtw.product", EventID: "missing-target"}
