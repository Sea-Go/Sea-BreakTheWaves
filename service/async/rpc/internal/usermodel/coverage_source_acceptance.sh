...
with open(log_path, 'w', encoding='utf-8') as output:
    result = subprocess.run(['go', 'test', '-mod=readonly', '-race', '-count=1', '-v',
                             '-run', '^TestCoverageVerifierRealDCAndRTW$', './service/async/rpc/internal/usermodel'],
...
wait "$fact_rtw_pid"
fact_rtw_pid=""
(cd "$fact_root" && GOFLAGS='-p=2' GOMAXPROCS=2 go vet ./service/async/rpc/internal/usermodel)
