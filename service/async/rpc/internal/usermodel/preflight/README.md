...
本包固定 `service/async/rpc/internal/usermodel/schema.go` 生成的 PostgreSQL 物理结构契约，用于在启用 SubjectRef v2 或迁移候选前审核 scoped 表结构和聚合计数。它不执行 DDL、不写业务行、不回显 DSN。逐表真实列、PK、FK、唯一键见 [物理结构清单](SCHEMA_INVENTORY.md)。
...
```bash
bash service/async/rpc/internal/usermodel/preflight/generate-contract.sh
python3 service/async/rpc/internal/usermodel/preflight/render-inventory.py
```
...
```bash
bash service/async/rpc/internal/usermodel/preflight/test-postgres.sh
```
