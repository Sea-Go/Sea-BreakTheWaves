...
func TestSubjectRefV2PGTimestampQuantization(t *testing.T) {
	...
	if !pgTimestampMatches(stored, source) {
		t.Fatal("pgx-truncated source timestamp rejected")
	}
	if pgTimestampMatches(stored.Add(time.Microsecond), source) {
		t.Fatal("adjacent PG microsecond accepted")
	}
}
