...
const (
	historyActionChatUser = "chat_user"
)

type travelHistoryRecorder struct {
	client        *graph.Client
	runID         string
	threadID      string
	userID        string
...
func newTravelHistoryRecorder(req travelStreamRequest) *travelHistoryRecorder {
	client := graph.GetClient()
	if client == nil || !client.IsEnabled() {
		return nil
	}
	return &travelHistoryRecorder{
		client:   client,
		runID:    req.RunID,
...
func (r *travelHistoryRecorder) Start(ctx context.Context, titleMessage, userMessage string) {
	...
	if err := r.client.AppendExplorationStep(ctx, graph.ExplorationStepNode{
		...
		ThreadID:    r.threadID,
		Seq:         0,
		ActionType:  historyActionChatUser,
		MessageRole: "user",
		Message:     userMessage,
		Status:      "done",
		CreatedAt:   now,
...
func (r *travelHistoryRecorder) RecordPlanningEvent(ctx context.Context, ev PublicPlanningEvent) {
	if r == nil || r.client == nil {
		return
	}
	...
	messageRole := ""
	if ev.Type == EventChatMessageDelta || ev.Type == EventPlanningError {
	...
	if ev.Type == EventPlanningCompleted {
		runUpdate.Status = "completed"
	} else if ev.Type == EventPlanningError {
	...
	if ev.Message != "" {
		...
		if ev.Type == EventChatMessageDelta {
