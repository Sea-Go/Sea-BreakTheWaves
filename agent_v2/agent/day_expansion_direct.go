...
func buildRouteBetweenPOIsDirect(ctx context.Context, from, to graph.POIInput, trace *TraceEmitter, noticeNodeID string) graph.RouteInput {
	...
	resp, err := tools.DrivingRoute(ctx, config.Cfg.Amap, tools.AmapDrivingRouteInput{
...
func routeInputFromDrivingResponse(resp tools.AmapResponse, from, to graph.POIInput) *graph.RouteInput {
