...
fact_tmp="$(mktemp -d "${TMPDIR:-/tmp}/sea-warehouse-favorite.XXXXXX")"
...
export WAREHOUSE_FAVORITE_ODS_OUTPUT="$fact_tmp/ods.jsonl"
python3 - "$fact_ready" "$fact_root" "$fact_tmp/warehouse-test.log" <<'PY'
import json
import os
import subprocess
import sys
...
env.update({
    ...
    'WAREHOUSE_FAVORITE_ODS_OUTPUT': os.environ['WAREHOUSE_FAVORITE_ODS_OUTPUT'],
    'GOFLAGS': '-p=2',
    'GOMAXPROCS': '2',
})
with open(log_path, 'w', encoding='utf-8') as output:
    result = subprocess.run(['go', 'test', '-mod=readonly', '-race', '-count=1', '-v',
                             '-run', '^TestRealRTWFavoriteWarehouse$', './internal/warehouse/favoritesource'],
                            cwd=root, env=env, stdout=output, stderr=subprocess.STDOUT,
                            check=False)
raise SystemExit(result.returncode)
PY
"${WAREHOUSE_FAVORITE_RUNTIME:?set locked CH/SeaweedFS/dbt runtime}/.venv/bin/python" "$fact_root/warehouse/favorite/acceptance.py" \
  --runtime "${WAREHOUSE_FAVORITE_RUNTIME:?set locked CH/SeaweedFS/dbt runtime}" \
  --output "$fact_tmp/ch-evidence" --ods "$fact_tmp/ods.jsonl" >"$fact_tmp/ch-acceptance.log" 2>&1
: > "$fact_release"
wait "$fact_rtw_pid"
fact_rtw_pid=""
(cd "$fact_root" && GOFLAGS='-p=2' GOMAXPROCS=2 go vet ./internal/warehouse/favoritesource)
(cd "$fact_root" && go mod verify)
(cd "$fact_root" && git diff --check)
rg '^--- PASS:|^PASS$|^ok[[:space:]]' "$fact_tmp/rtw-test.log" "$fact_tmp/warehouse-test.log" || true
cat "$fact_tmp/ch-acceptance.log"
