...
def parameters(revision):
    return dict(landing_schema='sea_fixture_landing', source_batches=['v1'] if revision == 1 else ['v1', 'v2'],
                ...
                label_revision=revision, splits=SPLITS)
...
def main():
    ...
    try:
        ...
        source_revision = 'sha256:' + hashlib.sha256(b''.join(
            path.read_bytes() for path in sorted(PROJECT.rglob('*'))
        ...
        epoch = publication.claim('generation_v1', 'attempt1', hashlib.sha256(json.dumps(parameters(1), sort_keys=True).encode()).hexdigest())
        build(args.dbt, endpoint, 'sea_gen_v1_a1', parameters(1), root / 'v1')
        ...
        build(args.dbt, endpoint, 'sea_gen_v1_replay', parameters(1), root / 'v1_replay')
        ...
        epoch_v2 = publication.claim('generation_v2', 'attempt1', hashlib.sha256(json.dumps(parameters(2), sort_keys=True).encode()).hexdigest())
        build(args.dbt, endpoint, 'sea_gen_v2_a1', parameters(2), root / 'v2')
        ...
        cancelled_epoch = publication.claim('cancelled_gen', 'attempt1', hashlib.sha256(json.dumps(parameters(2), sort_keys=True).encode()).hexdigest())
        ...
        report = dict(status='passed', scope='isolated_clickhouse_dbt_seaweedfs_synthetic_export',
                      ...
                      datasets={'v1': str(manifest_v1), 'v2': str(manifest_v2)},
                      s3_prefix=s3_prefix, s3_manifests={'v1': 'sea-fixture/sea_gen_v1_a1/manifest.json', 'v2': 'sea-fixture/sea_gen_v2_a1/manifest.json'},
                      v1_rows=len(old_rows), v2_rows=len(rows_v2), negative_cases=negative_cases,
                      assertions=['exact_redelivery_deduplicated', 'unexposed_not_negative', 'orphan_preserved_not_sample',
