...
fact_tmp="$(mktemp -d "${TMPDIR:-/tmp}/sea-favorite-feature.XXXXXX")"
...
finish() {
  # Release the isolated RTW fixture even when a downstream gate fails.
  : > "$fact_release"
...
# The ready file holds credentials. Python passes them only as child env;
# never source/eval it or print them into logs.
"${WAREHOUSE_FAVORITE_RUNTIME:?set locked CH/SeaweedFS/dbt runtime}/.venv/bin/python" \
  "$fact_root/warehouse/favorite_feature/acceptance.py" \
  --runtime "$WAREHOUSE_FAVORITE_RUNTIME" --output "$fact_tmp/cross-domain" \
  --ready "$fact_ready" --pg-dsn "$DATABASE_URL" >"$fact_tmp/cross-domain.log" 2>&1
: > "$fact_release"
wait "$fact_rtw_pid"
fact_rtw_pid=""
(cd "$fact_root" && GOFLAGS='-p=2' GOMAXPROCS=2 go vet ./internal/warehouse/favoritesource ./internal/warehouse/featurebaseline)
(cd "$fact_root" && go mod verify)
(cd "$fact_root" && git diff --check)
rg '^--- PASS:|^PASS$|^ok[[:space:]]' "$fact_tmp/rtw-test.log" "$fact_tmp/cross-domain/warehouse-test.log" "$fact_tmp/cross-domain/feature-test.log" || true
