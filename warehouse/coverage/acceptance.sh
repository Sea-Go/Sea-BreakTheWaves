...
fact_tmp="$(mktemp -d "${TMPDIR:-/tmp}/sea-coverage-publisher.XXXXXX")"
...
"${WAREHOUSE_FAVORITE_RUNTIME:?set locked CH/SeaweedFS/dbt runtime}/.venv/bin/python" \
  "$fact_root/warehouse/coverage/acceptance.py" --runtime "$WAREHOUSE_FAVORITE_RUNTIME" \
...
rg '^--- PASS:|^PASS$|^ok[[:space:]]' "$fact_tmp/rtw-test.log" "$fact_tmp/cross-domain/real-test.log" "$fact_tmp/cross-domain/two-user-test.log" || true
