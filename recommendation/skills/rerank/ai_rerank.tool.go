...
type AiRerankInput struct {
	UserID              string   `json:"user_id" jsonschema:"description=用户 ID,required"`
	UserIntent          string   `json:"user_intent" jsonschema:"description=本次推荐的意图描述"`
	CandidateArticleIDs []string `json:"candidate_article_ids" jsonschema:"description=候选文章 ID 列表,required"`
	TopK                int      `json:"topk" jsonschema:"description=返回条数,default=20"`
}
...
type AiRerankOutput struct {
	OverallConfidence float64      `json:"overall_confidence"`
	Ranked            []RankedItem `json:"ranked"`
}

func New(articleRepo *storage.ArticleRepo, memoryRepo *storage.MemoryRepo, memoryChunkRepo *storage.MemoryChunkRepo) tool.CallableTool {
	client := infra.NewAIClient()
	return function.NewFunctionTool(
		func(ctx context.Context, args AiRerankInput) (AiRerankOutput, error) {
			// 总 span
			...
			// 0.1) 参数校验/默认值
			{
				...
				if articleRepo == nil || memoryRepo == nil {
					msArgsValidate = time.Since(stepStart).Milliseconds()
					sp.End(zlog.StatusError, errors.New("ArticleRepo/MemoryRepo 未注入"), zap.Int64("step_ms", msArgsValidate))
					totalSp.End(zlog.StatusError, errors.New("ArticleRepo/MemoryRepo 未注入"))
					return AiRerankOutput{}, errors.New("ArticleRepo/MemoryRepo 未注入")
				}
				if len(args.CandidateArticleIDs) == 0 {
					msArgsValidate = time.Since(stepStart).Milliseconds()
					sp.End(zlog.StatusError, errors.New("candidate_article_ids 不能为空"), zap.Int64("step_ms", msArgsValidate))
					totalSp.End(zlog.StatusError, errors.New("candidate_article_ids 不能为空"))
					return AiRerankOutput{}, errors.New("candidate_article_ids 不能为空")
				}
			...
			var metas []storage.ArticleMeta
			{
				...
				var err error
				metas, err = articleRepo.GetArticlesByIDs(ctx, args.CandidateArticleIDs)
			...
			// 2) 取记忆
			var longMem, shortMem, periodicMem storage.UserMemory
			{
				stepStart := time.Now()
				...
				var e error
				longMem, memLongFound, e = memoryRepo.Get(ctx, args.UserID, storage.MemoryLongTerm, "")
			...
			}
			{
				...
				var e error
				shortMem, memShortFound, e = memoryRepo.Get(ctx, args.UserID, storage.MemoryShortTerm, "")
			...
			}
			{
				...
				var e error
				periodicMem, memPeriodicFound, e = memoryRepo.Get(ctx, args.UserID, storage.MemoryPeriodic, "d1")
				...
				if memoryChunkRepo != nil {
					...
					if queryText != "" {
						if vec, e := service.TextVector(ctx, queryText); e == nil && len(vec) > 0 {
							if memLongFound {
								if chunks, e := memoryChunkRepo.SearchMemoryChunks(ctx, args.UserID, storage.MemoryLongTerm, "", longMem.UpdatedAt, vec, 3); e == nil && len(chunks) > 0 {
									memLongStr = strings.Join(chunks, "\n")
								}
							}
							if memShortFound {
								if chunks, e := memoryChunkRepo.SearchMemoryChunks(ctx, args.UserID, storage.MemoryShortTerm, "", shortMem.UpdatedAt, vec, 3); e == nil && len(chunks) > 0 {
									memShortStr = strings.Join(chunks, "\n")
								}
							}
							if memPeriodicFound {
								if chunks, e := memoryChunkRepo.SearchMemoryChunks(ctx, args.UserID, storage.MemoryPeriodic, "d1", periodicMem.UpdatedAt, vec, 3); e == nil && len(chunks) > 0 {
			...
			if client == nil {
				return AiRerankOutput{}, errors.New("ai client is nil")
			}
		...
		function.WithName("ai_rerank_articles"),
		function.WithDescription("使用大模型对候选文章进行精排序：输入候选 article_id 列表，结合用户长期/短期/周期记忆输出最终排序与置信度。"),
	)
}
