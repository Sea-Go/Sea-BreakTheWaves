...
func WithProductionProviders(articleRepo *storage.ArticleRepo, poolRepo *storage.PoolRepo) Option {
	return func(rt *RecommendationRuntime) {
		...
		rt.registerAsyncHook(kafkaEventHook{})
	}
}
...
func (kafkaEventHook) OnEvent(ctx context.Context, event BehaviorEvent) (HookResult, error) {
	return HookResult{Decision: HookContinue}, kafka.PublishRecommendationEvent(ctx, kafka.RecommendationEventMessage{
		EventScope: kafka.RecommendationEventScope,
		EventID:    event.EventID,
		EventType:  event.EventType,
		RequestID:  event.RequestID,
		TraceID:    event.TraceID,
		TenantID:   event.TenantID,
		UserID:     valueOrFallback(event.User.UserID, event.User.AnonymousID),
