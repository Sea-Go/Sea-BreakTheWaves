...
## OBS-r3：Sink拒收与内部取消的终态

集成用户建模Graph时发现：`Runtime.Run`的Sink拒收会主动取消上游，但旧返回错误同时附加这个**内部**`context.Canceled`，使`runtime.run.finished`按取消而非原业务冲突/Outbox失败计数。现在区分Sink触发的内部取消与调用方/父Context取消：前者继续取消并排空框架通道，保留原Sink错误，记录`failed/RUN_SINK_FAILED`；后者仍记录`cancelled/CANCELLED`或超时。`TestSinkRejectionKeepsFailureOutcome`使用带Runner completion的假事件验证返回错误和真实JSON终态，既有HTTP模型取消/Sink错误测试仍通过。该修复仅校准项目Runtime的观测与返回，不等于修复锁定tRPC-Agent-Go v1.8.1在某些Graph错误事件上已复现的共享对象race，也不把用户建模普通Store阶段自动变成框架原生Span。
