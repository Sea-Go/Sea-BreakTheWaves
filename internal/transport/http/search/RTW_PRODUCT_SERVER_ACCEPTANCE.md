...
当前默认脚本中的 User RPC 是真实 gRPC 传输上的验收替身；`TestRealHTTPKnowledgeWorkflowWithUserCenter` 明确跳过，不能把这次结果写成真实 UserCenter 进程已联验。三路真实检索命中和真实模型质量仍待后续验收。
