...
`TestSignedScopeHTTPRejectsBeforeSourceAndAgent` 在隔离 HTTP/真实 RootSessionBoundary/真实 tRPC 单根 Graph 测试进程内，逐一注入缺/重复头、坏签名/编码、未知及缺字段、错受众/主体/模块/查询/深度/智能层、过期/未来/超长时窗；每个反例确认没有 SourceReader、模型和产品历史调用。正确签名继续产生真实 RootSessionBoundary 的成功响应和引用收据。`TestSignedScopeResolverRejectsShortKeyAndCopiesKey` 验证启动条件和密钥复制。此项状态为 **`LOCAL_VERIFIED`**，原因是签发 token 来自本地 fixture；RTW 实际 JWT/User RPC/当前人工发布/幂等产品 POST、真 RTW→BTW 双进程握手及 Collector 链路均未在本验收中证明。不能据此把 H02/J02/H07 整体标为 `ACCEPTED`。
