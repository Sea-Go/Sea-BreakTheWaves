...
func auditVersions(ctx context.Context, tx pgx.Tx, schema string, report *Report) error {
	...
	checks := []struct{ level, rule, table, sql string }{
		...
		{"L2", "missing_source_watermark", "usermodel_events", `SELECT count(*) FROM ` + q("events") + ` e WHERE e.source_sequence>0 AND NOT EXISTS (SELECT 1 FROM ` + q("watermarks") + ` w WHERE (w.authority_id,w.tenant_id,w.subject_id,w.producer,w.source_partition)=(e.authority_id,e.tenant_id,e.subject_id,e.producer,e.source_partition))`},
