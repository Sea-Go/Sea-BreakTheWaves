...
func (a *graphWorkflowAgent) checkAfterMacroPlanning(ctx context.Context, tripPlanID string, expectedTotalDays int, requirements ...workflowruntime.TravelRequirementSnapshot) error {
	...
	totalDayCount := 0
	var geoConstraint domaingeo.TravelGeoConstraint
	...
	var geoViolations []domaingeo.TravelGeoViolation
	...
	if len(geoViolations) > 0 {
		return &domaingeo.TravelGeoScopeError{Violations: geoViolations}
	}
...
func minMacroPhaseCount(expectedTotalDays int, requirements ...workflowruntime.TravelRequirementSnapshot) int {
