...
type UsageEmitter interface {
	EmitModelUsage(ctx context.Context, usage domainevents.PublicModelUsage, stage string)
}

type usageEmitterContextKey struct{}

func ContextWithUsageEmitter(ctx context.Context, emitter UsageEmitter) context.Context {
	return context.WithValue(ctx, usageEmitterContextKey{}, emitter)
}

func usageEmitterFromContext(ctx context.Context) UsageEmitter {
	if ctx == nil {
		return nil
	}
	if emitter, ok := ctx.Value(usageEmitterContextKey{}).(UsageEmitter); ok {
		return emitter
	}
	return nil
}
	...
	go func() {
		...
		for resp := range ch {
			...
			if resp != nil && resp.Done && resp.Usage != nil {
				...
				if emitter := usageEmitterFromContext(ctx); emitter != nil {
					emitter.EmitModelUsage(ctx, domainevents.PublicModelUsage{
