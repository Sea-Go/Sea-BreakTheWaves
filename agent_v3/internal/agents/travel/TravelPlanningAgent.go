...
import (
	...
	graphtools "agent_v3/internal/tools/graph"
	zhihutools "agent_v3/internal/tools/zhihu"
	workflowstages "agent_v3/internal/workflow/stages"
...
func TravelPlanningAgent() agentcore.Agent {
	...
	if graphClient != nil && graphClient.IsEnabled() {
		workflowAgent := workflowstages.NewGraphWorkflowAgent(workflowstages.Config{
			Coordinator:       newTravelPlanningTeam(),
			IntakeAgent:       newIntakeOnlyAgent(),
			GraphClient:       graphClient,
			ReviewAgents:      review.DefaultDayReviewAgents(),
			MacroAgentFactory: newMacroPlanningAgent,
		})
...
func newTravelPlanningTeam() agentcore.Agent {
	...
	guideTools := append(zhihutools.NewDefaultZhihuTools(), bilibilitools.NewDefaultBilibiliTools()...)
	graphTools := graphtools.NewDefaultGraphTools()
	if len(graphTools) > 0 {
	...
	tm, err := team.New(
		...
		[]agentcore.Agent{
			...
			review.ConstraintReviewAgent("trip"),
			review.ConstraintReviewAgent("phase"),
			review.ConstraintReviewAgent("month"),
			review.ConstraintReviewAgent("week"),
			review.ConstraintReviewAgent("poi"),
			Dili360Agent(),
		},
